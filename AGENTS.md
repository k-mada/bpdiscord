...
Source of truth for anyone — human or agent — working on this repository. Deliberately tool-neutral: `CLAUDE.md` is a one-line import of this file, so Claude Code and every other agent read the same instructions.
...
- **`MFLRosters`** is the named-roster parent — one row per roster, many per user. `lbusername` FKs to `Users` **`ON DELETE CASCADE`**; `UNIQUE (lbusername, name)` stops a user owning two rosters with the same name; a `CHECK` keeps the trimmed name 1–80 chars. A user may own up to **10** rosters — the cap is enforced in `dbCreateRoster` under a per-user `pg_advisory_xact_lock` so concurrent creates can't race past it. `is_official` marks the one roster ranked on the main "Standings" list; a **partial unique index** `mfl_rosters_one_official_per_user (lbusername) WHERE is_official` limits it to one per user (zero is allowed). The official switch is a clear-then-set inside `dbUpdateRoster`/`dbCreateRoster`'s transaction (`setOfficialRoster`): it clears the caller's current official before setting the new one, so moving official from roster A to B is one atomic call and never trips the index.
- **`MFLUserPicks`** is the roster edge, keyed on **`(roster_id, film_slug)`** — `roster_id` FKs to `MFLRosters` **`ON DELETE CASCADE`** (so delete flows user → rosters → picks), `film_slug` FKs to `MFLFilms` **`ON DELETE RESTRICT`** (deleting a rostered film is refused, so correcting a mistyped slug can't silently wipe rosters). The same film may appear in two of a user's rosters, but not twice in one. It carries **no `lbusername`** — the owner is reached through `MFLRosters`.
...
- `/api/stats` — public, DB-only reads powering the Stats page. Rate-limited 200 req / 5 min per IP (skipped outside production). `top-films/:year?` and `rating-deviation/:year?` share the optional Express-4 `:year?` shape (bare → all-time, `/:year` → that release year; 400 on a year outside 1870–2100). `rating-deviation` returns the films whose Discord-scoped average diverges most above (`over`) and below (`under`) `Films.lb_rating`, one winner per side expanding to ≤3 on an exact tie; the sign gate is server-side, so `over` holds only positive-deviation films. Min ratings: **20** all-time, **10** per-year — higher than top-films' 5 because it is a raw extremum with no Bayesian weighting to damp low-N noise.
...
- `/api/mfl` — Movie Fantasy League. The catalogue/member reads are **public**; the two writes are **admin only** and live under `/mfl/admin/*`.
  - `GET /leaderboard` — public. Returns **two ranked lists** `{ official, all }`: `all` is every roster (the "Sicko Mode" tab), `official` is only rosters marked `is_official` (the "Standings" tab). **One row per roster** (a user with several rosters appears several times), ranked by total points descending. Totals reuse `dbGetMFLMovies`'s aggregation — the same `MFLScoringTally` rows, grouped by `roster_id` instead of by film — so a roster's rank matches the films table. Only rosters that hold films appear; a roster whose picks are all unscored still ranks at 0. Ties get **competition rank** (equal totals share a rank, the next distinct total skips), computed server-side **per list** so each ranks 1..n within itself; rows order total desc, then roster name asc, then lbusername asc. Each row carries `rank`, `rosterId`, `name`, `lbusername`, `displayName`, `totalPoints` — the client renders `name (lbusername)`.
  - `GET /rosters/:rosterId` — **public**, read-only roster view: the roster's name, owner, and each film with its price and accumulated points, plus the roster total (same aggregation as `dbGetMFLMovies`, scoped to the roster). 404 when no such roster; an existing empty roster is a 200 with `picks: []`. Powers the `/mfl/roster/:rosterId` page the standings link to.
  - `GET /rosters`, `POST /rosters`, `GET /rosters/:rosterId/picks`, `PUT /rosters/:rosterId`, `DELETE /rosters/:rosterId` — **JWT-authed** roster CRUD. Identity resolves from the JWT (never the request body); every `:rosterId` route verifies the roster belongs to the caller and returns **404** otherwise, so an unowned id can't be probed. `POST` takes `{name, filmSlugs?, isOfficial?}` and returns `201 {rosterId}`; `PUT` takes `{name?, filmSlugs?, isOfficial?}` (at least one). `isOfficial: true` makes this the caller's official roster and clears the previous one in the same transaction; `false` clears it, leaving none. A duplicate name **or** the 10-roster cap is **409**; a slug not in `MFLFilms` is **404**; the account with no linked `lbusername` is **409**. Picks are replaced whole in one transaction, so a rejected submit leaves the previous picks intact. Integrity cap: ≤20 films per roster, no duplicate slug (roster size/budget are the client's rules). `GET /rosters` returns `{rosterId, name, isOfficial}` per roster.
  - `POST /admin/movie-score`, `DELETE /admin/movie-score/:scoringId` — both write `MFLScoringTally`, a film's award of one scoring metric, despite the legacy "scoring metric" naming that survived until bpdiscord-ayy. A repeat award hits the `mfl_scoring_tally_film_metric_key` constraint and returns **409**; `points_awarded` of 0 is legitimate, so the handler validates types rather than truthiness. Editing the metrics themselves is bpdiscord-s6t and does not exist yet.
  - The season catalogue in `MFLFilms` has **no write endpoint**. It is loaded and corrected through Supabase Studio's CSV import, so the app is a reader of that table only. The `mfl_films_price_non_negative` CHECK is therefore the sole guard on `price` — a Studio import runs no application validation.
...
Scan the diff for comment blocks over two lines that restate the code, and delete them.
...
A PR is read by a person. Keep it small enough that one can hold it in their head, and treat these as a prompt to think rather than a rule to obey — a mechanical rename can be 900 lines and trivially reviewable, while 24 unfamiliar files across two workspaces is not.

**Roughly 10 files or 400 added lines is the point to stop and reconsider.**

Two moments to check, not one:

1. **When planning.** If a task looks like it will exceed that, say so in the plan, propose a split, and either recommend one or ask which is preferred. Do not start and discover it.
2. **When scope changes mid-branch.** A branch that started small and grew through three rounds of feedback needs the same conversation. The estimate up front cannot catch this; only re-checking can.

Split along seams that stand alone. A server endpoint with its tests and no consumer yet is a reviewable PR; the screen that calls it is the next one. Later UI changes then land as small PRs against a merged base instead of more commits on an already-large one.

## Verify a push before reporting it

`git checkout -b` branches from wherever you are, and a preceding `git checkout main` can fail silently on an uncommitted file. Confirm what landed rather than assuming:

```bash
git branch --show-current      # the branch you think you are on
gh pr diff <n> --name-only     # the files the PR actually contains
```

A PR carrying files you did not touch means the branch was cut from the wrong base. Rebase it onto `main` and force-push with `--force-with-lease`.

## Keeping this file in sync

AGENTS.md is the source-of-truth for engineers (and AI agents) starting on the project. Doc-code drift is a real cost — e.g. a React 18 / React 19 mismatch caught during Stage 3 planning, where the docs said 18 but the codebase was already on 19.

Before opening any PR, check whether the changes affect anything documented here. If yes, update AGENTS.md as part of the **same PR**.
