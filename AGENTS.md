...
- **`MFLRosters`** is the named-roster parent — one row per roster, many per user. `lbusername` FKs to `Users` **`ON DELETE CASCADE`**; `UNIQUE (lbusername, name)` stops a user owning two rosters with the same name; a `CHECK` keeps the trimmed name 1–80 chars. A user may own any number of rosters (no cap).
