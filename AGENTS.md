...
- `/api/mfl` — Movie Fantasy League. The catalogue/member reads are **public**; the two writes are **admin only** and live under `/mfl/admin/*`.
  - `GET /leaderboard` — public. **One row per roster** (a user with several rosters appears several times), ranked by total points descending. Totals reuse `dbGetMFLMovies`'s aggregation — the same `MFLScoringTally` rows, grouped by `roster_id` instead of by film — so a roster's rank matches the films table. Only rosters that hold films appear; a roster whose picks are all unscored still ranks at 0. Ties get **competition rank** (equal totals share a rank, the next distinct total skips), computed server-side; rows order total desc, then roster name asc, then lbusername asc. Each row carries `rosterId`, `name`, `lbusername`, `displayName` — the client renders `name (lbusername)`.
