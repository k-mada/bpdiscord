...
- `/api/mfl` — Movie Fantasy League. The catalogue/member reads are **public**; the two writes are **admin only** and live under `/mfl/admin/*`.
  - `GET /leaderboard` — public. Ranks every member with picks by total points, descending. Totals reuse `dbGetMFLMovies`'s aggregation — the same `MFLScoringTally` rows, grouped by member instead of by film — so a member's rank matches the films table they see. Only members with picks appear; a member whose picks are all unscored still ranks at 0. Ties get **competition rank** (equal totals share a rank, the next distinct total skips), computed server-side; rows order total desc then lbusername asc.
