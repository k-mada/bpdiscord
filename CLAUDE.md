...
- `/api/mfl` — Movie Fantasy League. The four reads are **public**; the three writes are **admin only** and live under `/mfl/admin/*`.
  ...
  - `POST /admin/films/bulk` — upserts the season catalogue from rows a client parsed out of `films.csv`. Body is `{ rows, dryRun? }`, capped at 1000 rows (**413** past that). Rows use the CSV's snake_case column names (`title`, `film_slug`, `release_date` as mm/dd/yyyy, `price`); `release_date` and `price` are optional and a blank cell writes NULL. **All-or-nothing**: one bad row rejects the whole upload with **400** and writes nothing, so the catalogue only ever reflects a file that validated whole. A `dryRun` always answers **200** — reporting bad rows is what a preview is for — and returns the identical `accepted`/`rejected` arrays the commit would, because both call `validateFilmRows` in `controllers/mflFilmImport.ts`. Row numbers are 1-based into the submitted array, not CSV lines; the client owns the header offset.
