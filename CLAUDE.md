...
- `/api/mfl` — Movie Fantasy League. The four reads are **public**; the three writes are **admin only** and live under `/mfl/admin/*`.
  ...
  - `POST /admin/films/bulk` — upserts the season catalogue from rows a client parsed out of `films.csv`. Body is `{ rows, dryRun? }`, capped at 1000 rows (**413** past that). Rows use the CSV's snake_case column names (`title`, `film_slug`, `release_date` as mm/dd/yyyy, `price`); `release_date` and `price` are optional and a blank cell writes NULL, but a value that is neither text nor a finite number is a rejected row rather than a silent NULL. Slugs are lowercased on ingest — `film_slug` is a case-sensitive key that `MFLUserPicks` points at, so a mis-cased row would otherwise open a second catalogue entry no pick can reference. `price >= 0` is additionally enforced by the `mfl_films_price_non_negative` CHECK, because the row-level CRUD is a second writer. **All-or-nothing**: one bad row rejects the whole upload with **400** and writes nothing, so the catalogue only ever reflects a file that validated whole. A `dryRun` always answers **200** — reporting bad rows is what a preview is for — and returns the identical `valid`/`invalid` arrays the commit would (they name what validation found, never what the DB did, so emptying one on a failed commit would break exactly the property this endpoint exists for), because both call `validateFilmRows` in `controllers/mflFilmImport.ts`. Row numbers are 1-based into the submitted array, not CSV lines; the client owns the header offset.
