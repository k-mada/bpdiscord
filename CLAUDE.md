...
- Accessibility lint (`eslint-plugin-jsx-a11y`) runs at `warn` across `**/*.tsx`. New rules land at `warn` so pre-existing findings don't block; each a11y follow-up promotes the rules covering its own files to `error`. This is only safe because `yarn lint-js` is bare `eslint .` — adding `--max-warnings` would make every one of them blocking at once.
