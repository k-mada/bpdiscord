...
- `/api/mfl` — Movie Fantasy League. The four reads are **public**; the three writes are **admin only** and live under `/mfl/admin/*`.
  ...
  - `POST /admin/films/bulk` — upserts the season catalogue from CSV rows (`{ rows, dryRun? }`, max 1000). **All-or-nothing**: one bad row rejects the whole upload and writes nothing, so the catalogue only ever reflects a file that validated whole. A `dryRun` answers 200 even when rows are bad, and returns the same `valid`/`invalid` arrays the commit would — both call `validateFilmRows` in `controllers/mflFilmImport.ts`, which is the point of it being a separate module. Slugs are lowercased on ingest because `MFLUserPicks` references `film_slug` case-sensitively.
