...
// ===========================
// Shared functions (test cases kept in sync with client via ratingTestCases)
// ===========================

describe("parseRatingFromTitle", () => {
  it.each(parseRatingFromTitleCases)("$description", ({ input, expected }) => {
    expect(parseRatingFromTitle(input)).toBe(expected);
  });
});
...
describe("formatFilmsResponse", () => {
  ...
  it("includes ISO timestamp", () => {
    const result = formatFilmsResponse("alice", [], "test", "database") as { data: { timestamp: string } };
